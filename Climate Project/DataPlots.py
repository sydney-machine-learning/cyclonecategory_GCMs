        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                if int(row[col_names["No. of Cycl"]]) in wind_intensity_cnt:
                    ...
                    wind_intensity_sum[int(row[col_names["No. of Cycl"]])]+=int(row[col_names["Speed(knots)"]])
                else:
                    wind_intensity_cnt[int(row[col_names["No. of Cycl"]])]=1
                    wind_intensity_sum[int(row[col_names["No. of Cycl"]])]=int(row[col_names["Speed(knots)"]])
        wind_intensity_average = {}
        for cyclone_id in wind_intensity_cnt:
            wind_intensity_average[cyclone_id] = wind_intensity_sum[cyclone_id]/wind_intensity_cnt[cyclone_id]
    ...
    with open(filename) as csv_file:
        ...
        for row in csv_reader:
            ...
            if int(row[col_names["No. of Cycl"]]) in wind_intensity_variance:
                wind_intensity_variance[int(row[col_names["No. of Cycl"]])]+=(int(row[col_names["Speed(knots)"]])-wind_intensity_average[int(row[col_names["No. of Cycl"]])])**2
            else:
                wind_intensity_variance[int(row[col_names["No. of Cycl"]])]=(int(row[col_names["Speed(knots)"]])-wind_intensity_average[int(row[col_names["No. of Cycl"]])])**2
        for cyclone_id in wind_intensity_cnt:
            wind_intensity_variance[cyclone_id]/=wind_intensity_cnt[cyclone_id]
            wind_intensity_std[cyclone_id] = sqrt(wind_intensity_variance[cyclone_id])
        cyclone_data = []
        for cyclone_id in wind_intensity_average:
            cyclone_data.append([cyclone_id, wind_intensity_average[cyclone_id], wind_intensity_std[cyclone_id]])
        cyclone_data.sort()
        cyclone_ids = []
        ...
        fig, ax = plt.subplots()
        ax.bar(xpoints, wind_intensity_averages, yerr=wind_intensity_stds, align='center', alpha=0.5, ecolor='black', capsize=10)
...
def plot_mean_SD(filename):
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                if int(row[col_names["No. of Cycl"]]) not in cyclone_ids:
                    cyclone_ids[int(row[col_names["No. of Cycl"]])] = [float(row[col_names["Week_1_Mean"]]), float(row[col_names["Week_2_Mean"]]), float(row[col_names["Week_3_Mean"]]), float(row[col_names["Week_4_Mean"]]), float(row[col_names["Week_1_SD"]]), float(row[col_names["Week_2_SD"]]), float(row[col_names["Week_3_SD"]]), float(row[col_names["Week_4_SD"]])]
    l= []
    ...
    fig, ax = plt.subplots()
    ax.bar(xpoints, data_mean[0], yerr=data_std[0], align='center', alpha=0.5, ecolor='black', capsize=10)
